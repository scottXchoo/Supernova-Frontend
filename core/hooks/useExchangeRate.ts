...
export const useExchangeRate = (fromDenom: string, toDenom: string) => {
  ...
  const inputMulDecimal = Big(1).mul(Math.pow(10, fromDenomDecimal));
  ...
  }, [wasmClient, pairAddress, assetWithAmount]);
  ...
  const {
    ...
    queryKey: ["exchangeRate", { fromDenom, toDenom }],
