...
const Delegation = () => {
  ...
  const [{ delegate }, setIsModalOpen] = useRecoilState(modalsAtom);
  const setDelegateAmount = useSetRecoilState(delegateAmountAtom);
  const [inputAmount, setInputAmount] = useState<string>(DEFAULT_INPUT_VALUE);
  ...
  if (!delegate || !validator || !novaBalance || novaBalanceError) return null;
