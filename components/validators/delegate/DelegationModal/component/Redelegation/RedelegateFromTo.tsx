...
const RedelegateFromTo = () => {
  const closeAllModal = useResetRecoilState(modalsAtom);
  const [{ redelegateFrom }, setIsModalOpen] = useRecoilState(modalsAtom);
  const { data: novaAPR } = useCoinApy(APY_ZONE_NAME_NOVA);
  const redelegateToAddress = useRecoilValue(delegateOperatorAddressAtom);
  const redelegateToValidator = useRecoilValue(
    validatorFamily(redelegateToAddress),
  );
  const redelegateFromAddress = useRecoilValue(redelegateFromAddressAtom);
  const redelegateFromValidator = useRecoilValue(
    validatorFamily(redelegateFromAddress),
  );

  const delegation = useRecoilValue(delegationFamily(redelegateFromAddress));
  ...
  const onRedelegateButtonClick = async () => {
    ...
    const result = await redelegateMutation.mutateAsync({
      ...
      validatorFromAddress: redelegateFromAddress,
        ...
        <div className="relative md:px-8 px-5 md:py-7 py-5 md:pb-8 pb-6">
          <div className="mx-auto border-b border-white md:mb-6 md:pb-4 pb-2 mb-2">
            <h3 className="w-full text-white text-left font-semibold md:text-xl md:mb-4 mb-2 text-base">
              You&apos;re redelegate from
            </h3>
            <div className="flex mx-auto bg-gray-800 md:px-6 px-3 md:py-5 py-4 items-center md:rounded-xl rounded-lg md:mb-2">
              <div className="w-full">
                <div className="flex justify-between items-center md:pl-2 pl-2 w-full">
                  <h3 className="text-white text-left font-semibold text-lg w-1/2 md:w-1/3 md:text-xl md:mr-3 mr-2">
                    {redelegateFromValidator.moniker ||
                      makeEllipsisText(redelegateFromValidator.operatorAddress)}
                  </h3>
                  <div className="group grid w-1/2 md:w-2/3 justify-end">
                    <span className="text-xs font-bold text-white md:text-sm text-right transform scale-90 origin-bottom-right">
                      NOVA
                    </span>
                    <div className="group grid w-full justify-end">
                      <p className="font-semibold overflow-x-auto text-2xl text-right text-white md:text-3xl">
            ...
            <h3 className="w-full text-white text-left font-semibold md:text-xl text-base md:mb-2 mb-1">
              You&apos;re redelegate to
            </h3>
            <div className="flex mx-auto bg-gray-800 md:px-6 px-3 py-4 items-center md:rounded-xl rounded-lg md:mb-2 md:py-5 pr-5 md:pr-7">
              <div className="w-full">
...
export default RedelegateFromTo;
