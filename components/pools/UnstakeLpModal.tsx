...
import useUnstakeLpMutation from "core/hooks/pools/useUnstakeLpMutation";
import * as gtag from "lib/gtag";
import { ConfirmButton, InActivateButton } from "./Button";
import { unstakeLpModalAtom } from "core/state/pools/lpModalState";
import useStakedLpTokens from "core/hooks/pools/useStakedLpTokens";
import Big from "big.js";
import { denomByDisplayDenom, getPairInfo } from "core/utils/byDenomUtils";
import { useLpDecimal } from "core/hooks/useLiquidity";
...
const UnstakeLpModal = () => {
  ...
  const { data: stakedLpTokens } = useStakedLpTokens(pairInfo);

  const stakedLpTokensToBig = Big(stakedLpTokens?.amount || 0)
    .round(lpDecimal, Big.roundDown)
    .div(Big(10).pow(lpDecimal));

  const unstakeLpMutation = useUnstakeLpMutation(pairInfo);
  ...
  const {
    ...
    max: stakedLpTokensToBig.toFixed(lpDecimal, Big.roundDown),
  });
  ...
  }, [input, setUnstakeLpModal]);

  const handleUnstakeLpExecuteButton = async () => {
    if (!inputAmount) {
      return;
    }
    ...
    await unstakeLpMutation.mutateAsync({
                  ...
                  <div className="md:px-8 md:py-6 px-5 py-4">
                    {stakedLpTokensToBig.lte(0) && (
                      <div className="block bg-blue-500 rounded-lg md:rounded-xl items-center relative justify-center md:px-6 px-4 md:mb-4 mb-2 md:py-5 py-3">
                        <div className="md:text-base text-white font-medium text-xs mx-auto md:leading-tight leading-tight text-center">
                          No Tokens to unstake: Stake First
                        </div>
                      </div>
                    )}
                    ...
                    <div className="w-full flex items-center space-x-2  md:mb-4 mb-4">
                      {isOverMax ? (
                        ...
                        <ConfirmButton
                          onClick={handleUnstakeLpExecuteButton}
                          content="Confirm"
                        />
                      )}
                    </div>
