...
import { isVoteModalOpenAtom } from "core/state/proposal/vote/voteModal";
...
const VoteModal = ({ id }: { id: string }) => {
  const voteMutation = useVoteMutation(id);
  const [isOpen, setIsOpen] = useRecoilState(isVoteModalOpenAtom);
  ...
  const voteOptionButtonClicked = (option: VoteOption) => {
    const isSelected = option === voteOption
    setVoteOption(isSelected ? null : option);
  };
